...
| `rawBase64` | Corps de réponse HTTP original encodé en Base64, sous forme de chaîne Base64 brute. Doit être le seul format de la requête. Le type MIME se trouve dans `metadata.contentType`. |
...
| `pages`       | `boolean`                   | `false`           | Renvoyer également le markdown physique page par page dans le champ `pages` du document. Aucun coût supplémentaire.                                                                                                                                                   |
| `blocks`      | `boolean`                   | `false`           | Renvoyer également les blocs de mise en page typés par page (boîtes englobantes normalisées, types de blocs, ordre de lecture, étendues de caractères markdown) dans le champ `blocks` du document. Aucun coût supplémentaire.                                        |
| `pageMarkers` | `boolean`                   | `false`           | Annoter les sauts de page dans le markdown du document avec des marqueurs `<!-- page N -->` (uniquement entre les pages ; la numérotation peut ignorer les pages fusionnées par un saut — voir [Parse](/fr/features/parse#page-markers-pdf)). Aucun coût supplémentaire. |
...
Utilisez le point de terminaison `/v2/agent` pour effectuer une extraction autonome de données sur plusieurs pages. L’agent fonctionne de manière asynchrone : vous démarrez un job, puis interrogez périodiquement l’API pour récupérer les résultats.

[Agent](/fr/features/agent) est la référence canonique pour ce point de terminaison, y compris les traces d’exécution, les webhooks et la liste complète des paramètres.
...
| `model`                 | `string`  | `"spark-2"`       | Modèle d&#39;IA à utiliser. Les modèles Spark 1 sont obsolètes et redirigent actuellement vers `"spark-2"`.                                                                                                                                                                 |
| `effort`                | `string`  | *(non défini)*    | Budget de raisonnement : `"low"`, `"medium"` ou `"high"`. Chaque exécution s&#39;effectue sur `"spark-2"`, vous pouvez donc envoyer `effort` avec ou sans `model`.                                                                                                          |
...
Les motifs utilisent la syntaxe regex de Rust (de type RE2) : les assertions look-around et les références arrière ne sont donc pas prises en charge ; un motif qui ne compile pas est rejeté avec une erreur `400`. Chaque champ accepte au maximum 1000 motifs de 2000 caractères chacun, et `includePaths` et `excludePaths` réunis peuvent contenir au maximum 1000 motifs et 100 000 caractères. Pour un filtrage par mots-clés, envoyez un motif court par terme plutôt que de regrouper les termes dans une seule longue alternance.
...
<div id="allowing-firecrawl-to-scrape-your-website">
  ### Autoriser Firecrawl à scraper votre site web
</div>

* **robots.txt** : Firecrawl lit le fichier `robots.txt` avant de crawler un site et applique les règles définies pour le jeton user-agent `FirecrawlAgent` ainsi que celles définies pour `*`. Utilisez `FirecrawlAgent` pour autoriser ou interdire Firecrawl dans `robots.txt`.
* **User agent** : `FirecrawlAgent` est le jeton que Firecrawl utilise pour appliquer les règles du `robots.txt`. Ne partez pas du principe qu&#39;il correspond aussi à l&#39;en-tête `User-Agent` de la requête elle-même. Autorisez Firecrawl via une directive `robots.txt` plutôt qu&#39;avec une règle de pare-feu ou de WAF fondée sur une chaîne `User-Agent`.
* **Adresses IP** : Firecrawl n&#39;utilise pas d&#39;ensemble fixe d&#39;adresses IP sortantes.
