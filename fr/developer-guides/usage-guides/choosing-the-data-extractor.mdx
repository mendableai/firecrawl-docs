...
* **Modèle** : chaque exécution utilise `spark-2` (par défaut). Les modèles Spark 1 sont obsolètes et sont redirigés vers `spark-2`
...
result = app.agent(
    ...
    schema=schema,
    model="spark-2"  # la valeur par défaut ; les modèles Spark 1 sont dépréciés et redirigés vers "spark-2"
)
```
