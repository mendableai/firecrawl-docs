...
Pour qu’il y ait un hit de cache, ces paramètres doivent correspondre exactement entre la requête d’origine et les requêtes suivantes : `url`, `mobile`, `location`, `waitFor`, `blockAds`, `screenshot` (activé/désactivé et pleine page), ainsi que le mode proxy avancé.
