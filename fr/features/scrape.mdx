...
Vous scrapez un document à une URL, ou vous en analysez un depuis le disque ? Voici quand utiliser Scrape et quand utiliser [Parse](/fr/features/parse) :
...
<div id="response-metadata-and-status-codes">
  ### Métadonnées de réponse et codes d’état
</div>

Une réponse de scrape comporte deux niveaux d’état distincts, qu’il est important de ne pas confondre :

* **État de la requête API** : l’état HTTP de l’appel à l’API Firecrawl lui-même, ainsi que le booléen `success` dans le corps de la réponse. Une requête acceptée et traitée renvoie HTTP `200` avec `success: true`, même si la page cible a renvoyé un code autre que 2xx.
* **État de la page** (`data.metadata.statusCode`) : l’état HTTP renvoyé par le site web cible pour cette page (par ex. `200`, `301`, `404`, `403`, `500`). Il indique comment la page elle-même a répondu.

Le schéma fiable est donc le suivant : vérifiez d’abord que la requête a réussi (HTTP `200` + `success: true`), puis consultez `data.metadata.statusCode` pour connaître le résultat réel de la page. Firecrawl considère un `statusCode` de 2xx ou `304` comme un chargement de page réussi ; toute autre valeur signifie que la page ne s’est pas chargée correctement. Lorsqu’une page renvoie un état d’erreur, `data.metadata.error` peut fournir des détails supplémentaires.
...
* Base64 brut (`rawBase64`) - corps de réponse HTTP d’origine encodé en Base64 ; doit être le seul format de la requête
...
Le format `rawBase64` renvoie le corps de réponse HTTP d’origine, encodé en Base64. Le champ `rawBase64` contient une chaîne Base64 brute, et non un URI de données. Lisez le type MIME dans `metadata.contentType`. Demandez uniquement `rawBase64`, car l’API le rejette avec tout autre format. La réponse ne contient aucun champ `markdown`, `html` ou `rawHtml`, et ne contient que le fichier unique à cette URL, sans CSS, images ni autres sous-ressources.
