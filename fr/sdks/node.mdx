...
import AgentWithSchemaNode from '/snippets/fr/v2/agent/with-schema/js.mdx'
import AgentStatusNode from '/snippets/fr/v2/agent/status/js.mdx'
...
Chaque exécution enregistre également une trace d’exécution et des instantanés de sortie, que vous pouvez consulter avec `getAgentTrace` et `getAgentSnapshot`. Consultez [Agent](/fr/features/agent) pour le schéma des événements et la liste complète des paramètres.
