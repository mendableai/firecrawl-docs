...
Une requête qui envoie l’un de ces filtres sans définir `sources` n’obtient donc aucun résultat `doc`. Sa réponse ne contient que des éléments du dépôt : les types `issue`, `pull_request` et `readme`, car la partie documentation de l’index n’a jamais été interrogée. C’est le comportement prévu, et non un problème d’indexation.

Pour conserver les résultats de documentation, supprimez les filtres de dépôt. Vous pouvez également définir le périmètre de la partie documentation avec `sources`, puis consulter l’écho `sources` dans la réponse pour confirmer que l’identifiant est indexé.
