...
* **Modelo**: Cada execução usa `spark-2` (o padrão). Os modelos Spark 1 foram descontinuados e são direcionados para `spark-2`
...
result = app.agent(
    ...
    schema=schema,
    model="spark-2"  # o padrão; os modelos Spark 1 estão descontinuados e são redirecionados para "spark-2"
)
```
