...
| `pageMarkers` | `boolean`                   | `false`         | Anota quebras de página no markdown do documento com marcadores `<!-- page N -->` (somente entre páginas; a numeração pode pular páginas mescladas em uma quebra — consulte [Parse](/pt-BR/features/parse#page-markers-pdf)). Sem custo adicional. |
...
[Agent](/pt-BR/features/agent) é a referência canônica para este endpoint, incluindo rastros de execução, webhooks e a lista completa de parâmetros.
...
Os padrões usam a sintaxe de regex do Rust (estilo RE2), portanto look-around e retrorreferências não são suportados; um padrão que não compila é rejeitado com um `400`. Cada campo aceita no máximo 1000 padrões de até 2000 caracteres cada, e `includePaths` e `excludePaths` juntos podem conter no máximo 1000 padrões e 100.000 caracteres. Para filtragem baseada em palavras-chave, envie um padrão curto por termo em vez de combinar os termos em uma única alternância longa.
...
<div id="allowing-firecrawl-to-scrape-your-website">
  ### Permitindo que o Firecrawl faça scraping do seu site
</div>

* **robots.txt**: o Firecrawl lê o `robots.txt` antes de rastrear um site e aplica as regras definidas para o token de user-agent `FirecrawlAgent`, além das definidas para `*`. Use `FirecrawlAgent` para permitir ou bloquear o Firecrawl no `robots.txt`.
* **User agent**: `FirecrawlAgent` é o token que o Firecrawl usa como referência ao aplicar as regras do `robots.txt`. Não presuma que ele seja também o header `User-Agent` da requisição em si. Libere o Firecrawl com uma diretiva no `robots.txt`, e não com uma regra de firewall ou WAF que faça correspondência com uma string de `User-Agent`.
* **Endereços IP**: o Firecrawl não utiliza um conjunto fixo de IPs de saída.
