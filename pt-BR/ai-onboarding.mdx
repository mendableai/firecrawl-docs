...
Se você está desenvolvendo com IA, a Firecrawl oferece vários recursos para melhorar sua experiência. A Firecrawl inclui **skills** — pacotes de conhecimento autocontidos que agentes de codificação com IA descobrem e usam automaticamente. Com um único comando de instalação, os agentes recebem skills de CLI para trabalhar com a web em tempo real e skills de fluxo de trabalho para gerar entregas repetíveis. As skills de build para integrar a Firecrawl ao código da aplicação são uma etapa separada (`firecrawl setup build`). Usuários do Firecrawl podem obter uma chave de API de duas maneiras. Veja [Obter credenciais](#get-credentials) abaixo.

* [Obter credenciais](#get-credentials)
* [Skills de CLI e skill de agente](#cli)
...
As **skills** do Firecrawl são pacotes de conhecimento autocontidos que agentes de codificação de IA, como Codex, Claude Code, Cursor e OpenCode, descobrem e usam automaticamente. Um único comando de instalação configura as ferramentas de CLI para trabalho web em tempo real e as skills de fluxo de trabalho para produzir entregas repetíveis. As skills de build para integrar o Firecrawl ao código da aplicação são instaladas com `firecrawl setup build`:

```bash
npx -y firecrawl-cli@latest init --all --browser
```

* `--all` pula a seleção de agentes e inicializa todos os agentes detectados
* `--browser` abre o navegador automaticamente para autenticação no Firecrawl
* `--skip-auth`, `--skip-install`, `--skip-skills` e `--agent <name>` também estão disponíveis. Veja `firecrawl init --help`.
...
```bash
firecrawl setup skills      # CLI / skills principais
firecrawl setup build       # skills de build
...
As skills do Firecrawl cobrem três categorias. O `init` instala as skills de CLI e de fluxo de trabalho. Execute `firecrawl setup build` para as skills de build. Cada segmento fica em seu próprio repositório para poder evoluir de forma independente:

* [`firecrawl/cli`](https://github.com/firecrawl/cli) — skills de CLI para trabalho web em tempo real
* [`firecrawl/skills`](https://github.com/firecrawl/skills) — skills de build para integração com aplicações
* [`firecrawl/firecrawl-workflows`](https://github.com/firecrawl/firecrawl-workflows) — skills de fluxo de trabalho para entrega repetível

**Skills de CLI** — para trabalho web em tempo real durante uma sessão do agente:

| Skill                       | Purpose                                                  |
| --------------------------- | -------------------------------------------------------- |
| `firecrawl`                 | Fluxo geral de comandos da CLI                           |
...
| `firecrawl-monitor`         | Detectar mudanças no site e receber notificações         |
| `firecrawl-parse`           | Converter documentos locais em markdown                  |
| `firecrawl-download`        | Baixar um site como arquivos locais                      |
| `firecrawl-research-index`  | Buscar papers, ler trechos e seguir citações             |
| `firecrawl-developer-index` | Buscar issues, pull requests, READMEs e documentação     |

A instalação acima inclui ambas as skills de índice. Para adicionar apenas a de busca de papers a uma configuração existente — vale a pena quando o trabalho envolve literatura biomédica ou científica — execute:
...
As skills de CLI e de fluxo de trabalho vêm da instalação acima. As skills de build são uma etapa separada: `firecrawl setup build`. A diferença está no que acontece depois:
...
<Steps>
  ...
  <Step title="Integração com aplicação (skills de Build)">
    Use esta opção quando estiver criando uma aplicação, agente ou fluxo de trabalho que chama a API do Firecrawl via código. Instale as skills de build com `firecrawl setup build`. Elas ajudam a escolher o endpoint certo, configurar o SDK e executar um smoke test.
...
<AccordionGroup>
  <Accordion title="Search — descubra páginas por consulta" icon="magnifying-glass">
    ...
    **Quando usar:** Pesquisa geral na web, busca de documentação, análise competitiva, resposta a perguntas que exigem informações atualizadas da web.

    **Quando não usar:** Literatura científica. Para encontrar artigos, ler trechos de um deles ou seguir citações, use o [Research Index](/pt-BR/features/research) — as ferramentas MCP `firecrawl_research_*`, `firecrawl research search-papers` na CLI ou a habilidade `firecrawl-research-index`. A busca retorna snippets de páginas da web; o Research Index retorna registros de artigos com resumos completos.
  </Accordion>
