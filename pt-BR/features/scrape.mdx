...
Fazendo scraping de um documento em uma URL ou analisando um arquivo do disco? Veja aqui quando usar o Scrape e quando usar o [Parse](/pt-BR/features/parse):
...
<Info>
  Cada scraping consome 1 crédito. Créditos adicionais são cobrados para certas opções: modo JSON custa 4 créditos adicionais por página, os formatos question e highlights custam 4 créditos adicionais por página por formato, a ocultação de PII custa 4 créditos adicionais por página e o processamento de PDFs custa 1 crédito por página de PDF, e a extração de áudio ou vídeo custa 4 créditos adicionais por página.
</Info>
...
<div id="response-metadata-and-status-codes">
  ### Metadados da resposta e códigos de status
</div>

Há duas camadas distintas de status em uma resposta de scraping, e é importante diferenciá-las:

* **Status da solicitação de API**: o status HTTP da própria chamada à API do Firecrawl, juntamente com o booleano `success` no corpo da resposta. Uma solicitação aceita e processada retorna HTTP `200` com `success: true`, mesmo que a página de destino tenha retornado um código diferente de 2xx.
* **Status da página** (`data.metadata.statusCode`): o status HTTP retornado pelo website de destino para essa página (por exemplo, `200`, `301`, `404`, `403`, `500`). É isso que indica como a própria página respondeu.

O padrão confiável é: primeiro, confirme que a solicitação foi bem-sucedida (HTTP `200` + `success: true`); depois, verifique `data.metadata.statusCode` para saber o resultado real da página. O Firecrawl considera um `statusCode` de 2xx ou `304` como um carregamento de página bem-sucedido; qualquer outro valor significa que a página não foi carregada corretamente. Quando uma página retorna um status de erro, `data.metadata.error` pode incluir detalhes adicionais.
...
* Base64 bruto (`rawBase64`) - corpo da resposta HTTP original codificado em Base64; deve ser o único formato na solicitação
* Captura de tela (`screenshot`, com opções como `fullPage`, `quality`, `viewport`) — as URLs das capturas de tela expiram após 24 horas
* Links (`links`)
* JSON (`json`) - resultado estruturado
...
O formato `rawBase64` retorna o corpo da resposta HTTP original codificado em Base64. O campo `rawBase64` contém uma string Base64 simples, não uma URI de dados. Leia o tipo MIME em `metadata.contentType`. Solicite somente `rawBase64`, pois a API o rejeita junto com qualquer outro formato. A resposta não tem campos `markdown`, `html` ou `rawHtml` e contém apenas o arquivo único dessa URL, sem CSS, imagens ou outros sub-recursos.
