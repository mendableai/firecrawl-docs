---
title: 'Mapa'
description: 'Insira um site e obtenha todas as URLs dele — extremamente rápido'
og:title: "Mapa | Firecrawl"
og:description: "Vá de uma única URL para um mapa de todo o site"
icon: "map"
version: "v2"
sidebarTitle: "Mapa"
---

import InstallationPython from "/snippets/pt-BR/v2/installation/python.mdx";
import InstallationJavaScript from "/snippets/pt-BR/v2/installation/js.mdx";
import InstallationCLI from "/snippets/pt-BR/v2/installation/cli.mdx";
import MapPython from "/snippets/pt-BR/v2/map/base/python.mdx";
import MapJavaScript from "/snippets/pt-BR/v2/map/base/js.mdx";
import MapCURL from "/snippets/pt-BR/v2/map/base/curl.mdx";
import MapCLI from "/snippets/pt-BR/v2/map/base/cli.mdx";
import MapResponse from "/snippets/pt-BR/v2/map/base/output.mdx";
import MapLocationPython from "/snippets/pt-BR/v2/map/location/python.mdx";
import MapLocationNode from "/snippets/pt-BR/v2/map/location/js.mdx";
import MapLocationCURL from "/snippets/pt-BR/v2/map/location/curl.mdx";
import PlaygroundCTA from "/snippets/pt-BR/shared/playground-cta-map.mdx";

<div id="introducing-map">
  ## Apresentando o /map
</div>

A forma mais simples de ir de uma única URL para um mapa de todo o site. Isso é extremamente útil para:

* Quando você precisa pedir ao usuário final que escolha quais links devem ser coletados
* Quando você precisa saber rapidamente quais links existem em um site
* Quando você precisa coletar páginas de um site relacionadas a um tópico específico (use o parâmetro `search`)
* Quando você só precisa coletar páginas específicas de um site

<PlaygroundCTA />

<div id="mapping">
  ## Mapeamento
</div>

<div id="map-endpoint">
  ### endpoint /map
</div>

Usado para mapear uma URL e obter as URLs do site. Retorna a maioria dos links presentes no site.

As URLs são descobertas principalmente a partir do sitemap do site, complementadas com resultados de SERP (páginas de resultados de mecanismos de busca) e páginas rastreadas anteriormente para melhorar a cobertura. Você pode controlar o comportamento do sitemap com o parâmetro `sitemap`.

<div id="installation">
  ### Instalação
</div>

<CodeGroup>
  <InstallationPython />

  <InstallationJavaScript />

  <InstallationCLI />
</CodeGroup>

<div id="usage">
  ### Uso
</div>

<CodeGroup>
  <MapPython />

  <MapJavaScript />

  <MapCURL />

  <MapCLI />
</CodeGroup>

<Info>
  Cada chamada de `map` consome 1 crédito por execução, independentemente do número de URLs retornadas. Por exemplo, definir `limit` como 100.000 ainda usa 1 crédito.
</Info>

<div id="response">
  ### Resposta
</div>

Os SDKs retornarão o objeto de dados diretamente. O cURL retornará o payload exatamente como mostrado abaixo.

<MapResponse />

<Warning>
  Título e descrição nem sempre estão presentes, pois depende do site.
</Warning>

<div id="map-with-search">
  #### Mapear com busca
</div>

Usar o parâmetro `search` no Map permite procurar URLs específicas dentro de um site.

```bash cURL
curl -X POST https://api.firecrawl.dev/v2/map \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer SUA_CHAVE_DE_API' \
  -d '{
    "url": "https://firecrawl.dev",
    "search": "docs"
  }'
```

A resposta será uma lista ordenada da mais relevante para a menos relevante.

```json
{
  "status": "sucesso",
  "links": [
    {
      "url": "https://docs.firecrawl.dev",
      "title": "Docs do Firecrawl",
      "description": "Documentação do Firecrawl",
    },
    {
      "url": "https://docs.firecrawl.dev/sdks/python",
      "title": "SDK do Firecrawl para Python",
      "description": "Documentação do SDK do Firecrawl para Python"
    },
    ...
  ]
}
```

<div id="location-and-language">
  ## Localização e idioma
</div>

Especifique o país e os idiomas preferidos para obter conteúdo relevante com base no seu local de destino e nas preferências de idioma, de forma semelhante ao endpoint /scrape.

<div id="how-it-works">
  ### Como funciona
</div>

Ao definir as configurações de localização, o Firecrawl usará um proxy apropriado, se disponível, e emulará o idioma e o fuso horário correspondentes. Por padrão, a localização é definida como “US” se nada for especificado.

<div id="usage-2">
  ### Uso
</div>

Para usar as configurações de localização e idioma, inclua o objeto `location` no corpo da requisição com as seguintes propriedades:

* `country`: código de país ISO 3166-1 alfa-2 (por exemplo, &#39;US&#39;, &#39;AU&#39;, &#39;DE&#39;, &#39;JP&#39;). Padrão: &#39;US&#39;.
* `languages`: um array de idiomas e localidades preferenciais para a requisição, em ordem de prioridade. Padrão: o idioma da localização especificada.

<CodeGroup>
  <MapLocationPython />

  <MapLocationNode />

  <MapLocationCURL />
</CodeGroup>

Para mais detalhes sobre as localizações compatíveis, consulte a [documentação de Proxies](/pt-BR/features/proxies).

<div id="considerations">
  ## Considerações
</div>

Este endpoint prioriza a velocidade, portanto, talvez não capture todos os links do site. Ele se baseia principalmente no sitemap do site, complementado por dados de rastreamento em cache e resultados de mecanismos de busca. Para obter uma lista de URLs mais completa e atualizada, considere usar o endpoint [/crawl](/pt-BR/features/crawl).

Se você precisa de uma descoberta recursiva mais completa, com registros auditáveis de resultados e erros em vez de uma aproximação rápida, use o [/crawl](/pt-BR/features/crawl), que descobre páginas percorrendo links de forma recursiva e também pelo sitemap, além de informar o que retornou e o que não conseguiu buscar — consulte [Execução e contabilização de resultados](/pt-BR/features/crawl#execution-and-result-accounting). Para obter os links de uma página específica, solicite o [formato `links`](/pt-BR/features/scrape#scrape-formats) no [/scrape](/pt-BR/features/scrape).

> Você é um agente de IA que precisa de uma chave de API do Firecrawl? Consulte [firecrawl.dev/agent-onboarding/SKILL.md](https://www.firecrawl.dev/agent-onboarding/SKILL.md) para ver instruções de onboarding automatizado.