...
description: 'Detecte e monitore mudanças em conteúdo da web entre scrapings'
og:title: "Rastreio de mudanças | Firecrawl"
sidebarTitle: "Rastreio de mudanças"
og:description: "Detecte e monitore mudanças em conteúdo da web entre scrapings"
---

![Rastreio de mudanças](/images/launch-week/lw3d12.webp)

O rastreio de mudanças compara o conteúdo atual de uma página com o da última vez em que você fez o scraping. Adicione `changeTracking` ao seu array de `formats` para detectar se uma página é nova, inalterada ou modificada e, opcionalmente, obter um diff estruturado do que mudou.
...
O rastreio de mudanças é a primitiva de diff por scraping. Se você quiser que o Firecrawl execute as verificações em um agendamento e o notifique por webhook, email ou Slack quando algo mudar, use o [monitoramento](/pt-BR/features/monitoring) em vez de chamar o `/scrape` por conta própria em intervalos regulares.
...
<div id="scheduling-change-tracking">
  ## Agendando o rastreio de mudanças
</div>

O rastreio de mudanças é mais útil quando você faz scraping em uma agenda regular. O [monitoramento](/pt-BR/features/monitoring) executa esse agendamento para você, com intervalos de até 5 minutos, e envia notificações por webhook, email ou Slack. Use as opções abaixo quando quiser gerenciar o agendamento por conta própria com cron, agendadores em nuvem ou ferramentas de fluxo de trabalho.
...
<Warning>
  O formato `markdown` deve sempre ser incluído junto com `changeTracking`. O rastreamento de mudanças compara páginas com base no conteúdo em markdown.
</Warning>
