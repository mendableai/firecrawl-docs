...
O endpoint `/extract` simplifica a coleta de dados estruturados de qualquer número de URLs ou de domínios inteiros. Forneça uma lista de URLs, opcionalmente com curingas (por exemplo, `example.com/*`), e um prompt ou schema descrevendo as informações desejadas. O Firecrawl cuida dos detalhes de rastreamento, analisar e agregar conjuntos de dados, grandes ou pequenos.

<Info>Simplificamos a cobrança: agora o Extract usa créditos, assim como os outros endpoints. Cada crédito equivale a 15 tokens. Veja [Cobrança](/pt-BR/billing) para entender como os créditos se relacionam com o preço dos planos; scrape e rastreamento são cobrados a 1 crédito por página ([Custo do Scrape](/pt-BR/features/scrape#scraping-a-url-with-firecrawl)).</Info>
...
Simplificamos a cobrança: o Extract agora usa créditos, assim como os demais endpoints. Cada crédito equivale a 15 tokens. Consulte [Cobrança](/pt-BR/billing) para saber como os créditos se relacionam com o preço dos planos; scrape e rastreamento são cobrados a 1 crédito por página ([Custo do Scrape](/pt-BR/features/scrape#scraping-a-url-with-firecrawl)).
