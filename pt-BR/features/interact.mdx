...
* **Navegue pela paginação** — avance de página e colete cada conjunto de resultados, reutilizando uma única sessão. Veja [interagir usando prompts](#interact-via-prompting) e [ciclo de vida da sessão](#session-lifecycle).
* **Preencha e envie um formulário** — digite nos campos e envie, a partir de um prompt ou de código Playwright. Veja [interagir usando prompts](#interact-via-prompting) e [Executando código](#running-code).
* **Reutilize uma sessão autenticada** — salve o estado do navegador em um perfil nomeado e carregue-o em scrapings posteriores. Veja [Perfis persistentes com Scrape + interagir](#persistent-profiles-with-scrape-+-interact).
* **Inicie uma sessão sem fazer scraping antes** — abra um navegador independente, controlado diretamente por você. Veja [Browser Sandbox](/pt-BR/features/browser).
