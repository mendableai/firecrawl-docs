...
### Using Tags for Separate Change Tracking Histories

The `tag` parameter allows you to create separate "branches" of change tracking history. This is useful for separating different environments or contexts:

```javascript
// Scrape with production tag
const prodResult = await app.scrapeUrl('https://example.com', {
  formats: ['markdown', 'changeTracking'],
  changeTrackingOptions: {
    tag: 'production'
  }
...
// Scrape with staging tag - will not be compared against production scrapes
const stagingResult = await app.scrapeUrl('https://example.com', {
  formats: ['markdown', 'changeTracking'],
  changeTrackingOptions: {
    tag: 'staging'
  }
...
// Both will have changeStatus: "new" initially since they use different tags
console.log(prodResult.changeTracking.changeStatus); // "new"
console.log(stagingResult.changeTracking.changeStatus); // "new"
```

**Key points about tags:**
- Tags create completely separate change tracking histories
- Scrapes with different tags will never be compared against each other
- If no tag is specified, the default tag (`null`) is used
- Tags are scoped to your team/API key
- Useful for separating environments (production, staging, development)
- Can be used to track different versions or configurations of the same content
...
```python
# Scrape with production tag
prod_result = app.scrape_url('https://example.com',
    formats=['markdown', 'changeTracking'],
    change_tracking_options=ChangeTrackingOptions(
        tag='production'
    )
...
# Scrape with staging tag - will not be compared against production scrapes
staging_result = app.scrape_url('https://example.com',
    formats=['markdown', 'changeTracking'],
    change_tracking_options=ChangeTrackingOptions(
        tag='staging'
    )
...
# Both will have changeStatus: "new" initially since they use different tags
print("Production status:", prod_result.changeTracking.changeStatus)  # "new"
print("Staging status:", staging_result.changeTracking.changeStatus)  # "new"
```
...
- **Matching Previous Scrapes**: Previous scrapes to compare against are currently matched on the source URL, the team ID, the `markdown` format, and the `tag` parameter.
  ...
  - Scrapes with different `tag` values will never be compared against each other - they maintain separate change tracking histories.
  ...
  - Compared pages will also be compared against previous scrapes that only have the `markdown` format without the `changeTracking` format (as long as they use the same tag).
...
const result = await app.scrapeUrl('https://example.com/product', {
  ...
  changeTrackingOptions: {
    ...
    tag: 'product-monitoring' // Track product changes separately
  }
});
