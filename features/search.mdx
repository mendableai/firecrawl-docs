...
To perform searches without scraping content (which consumes fewer credits), simply provide an empty `formats` array or omit the `scrapeOptions` entirely. This returns only the basic search results (title, URL, description) without any scraped content:
...
**Cost Impact**: When `formats` is empty, you only pay for the search operation itself. No additional credits are consumed for content scraping.

### PDF Pricing Optimization

When search results include PDF files, you can control billing costs using the `parsePDF` parameter:

- **Default (`parsePDF: true`)**: 1 credit per PDF page
- **Optimized (`parsePDF: false`)**: 1 credit total per PDF document
...
**Cost Comparison**: A 50-page PDF would cost 50 credits with `parsePDF: true` (default) but only 1 credit with `parsePDF: false`. The trade-off is that with `parsePDF: false`, you get the PDF in base64 encoding instead of extracted text content.
