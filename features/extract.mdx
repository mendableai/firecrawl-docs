...
**Important:** The `/extract` endpoint is asynchronous. When you submit an extraction request, you receive a job ID immediately, then need to poll for results. There are two ways to handle this:

1. **SDK Auto-polling (Recommended)**: Use `app.extract()` - the SDK automatically polls for completion and returns final results
2. **Manual Async Control**: Use `app.async_extract()` + `app.get_extract_status()` for custom polling logic
3. **Direct API**: POST to start extraction → GET with job ID to retrieve results
...
### SDK Auto-polling Examples (Recommended)

The SDK methods below automatically handle the async polling for you and return the final results:
...
### Direct API Usage (Two-Step Process)

When using the API directly, you must first start the extraction, then poll for results:
...
### Initial Response (Job ID)

When you start an extraction, you receive a job ID:

<ExtractStatusAsync />

### Final Response (After Completion)

Once the extraction completes, you get the structured data:

<ExtractOutput />

## Manual Async Control

For custom polling logic or integration with job queues, use the async methods to get immediate job IDs:
...
<Note>
  Job status endpoints only work for jobs in progress or recently completed (within 24 hours).
</Note>
