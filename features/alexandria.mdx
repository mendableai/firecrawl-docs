...
An ordinary scrape can also request domain-matched tools. Add `domainTools: true` (default off) to a `/v2/scrape` request; if the team has Exchange access, the response includes `data.tools` — tools matched to the scraped page's domain, in the same shape as the tools returned by search.
...
Other Exchange refusals are relayed as-is, with the provider's status and `code` (for example, 402 `credit_budget_exceeded`), and nothing was executed.
