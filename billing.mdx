...
<Note>
**Crawl pre-flight credit check:** Before a crawl job starts, Firecrawl compares your remaining credit balance against the `limit` for the job. If the balance is lower than `limit`, the request is not rejected. Firecrawl lowers the effective limit to what your balance covers and starts the crawl with that smaller limit, so the job stops earlier than the `limit` you asked for. A 402 comes back only when the lowered limit is not a positive number, which means the balance no longer covers any pages, or when a per-API-key spend limit denies the lowered request as well. Omitting `limit` works the same way: the default of **10,000** is lowered to fit the balance rather than requiring 10,000 credits up front. Pass an explicit `limit` when you want the job to stop at a number of pages you chose yourself (e.g., `limit: 100`).
</Note>
