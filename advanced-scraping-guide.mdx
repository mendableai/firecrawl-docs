...
description: "Configure scrape options, browser actions, crawl, map, and the agent endpoint with Firecrawl's full API surface."
og:title: "Advanced Scraping Guide | Firecrawl"
og:description: "Configure scrape options, browser actions, crawl, map, and the agent endpoint with Firecrawl's full API surface."
---
...
- **User agent**: Firecrawl does not send `FirecrawlAgent` in the `User-Agent` header of its page requests, so firewall and WAF rules that match on that token will not match Firecrawl traffic.
