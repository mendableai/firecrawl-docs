---
title: "CLI"
description: "Firecrawl スキルは、Codex、Claude Code、Cursor、OpenCode などの AI エージェントが CLI 経由で Firecrawl を利用するための手軽な方法です。"
...
| `--categories <categories>`  | カテゴリで絞り込む: `research`、`pdf`、`developer` (カンマ区切り)                          |
...
| `--model <model>`           | 使用するモデル。デフォルトは、すべての実行で使用されるモデルである`spark-2`です。Spark 1モデルは非推奨であり、`spark-2`にルーティングされます |
