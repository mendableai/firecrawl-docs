...
`agent` メソッドを使用して、調査または抽出タスクをエージェントに渡します。このメソッドには、`prompt`、出力の形式を指定する任意の `schema`、実行で消費できるクレジットの上限を設定する `max_credits` を指定します。

<AgentWithSchemaPython />

エージェントの実行は非同期です。`start_agent` を使用するとジョブ ID がすぐに返され、その後 `get_agent_status` でポーリングできます。

<AgentStatusPython />

各実行では、実行トレースと出力スナップショットも記録されます。これらは `get_agent_trace` と `get_agent_snapshot` で取得できます。イベントスキーマとすべてのパラメータについては、[Agent](/ja/features/agent) を参照してください。
