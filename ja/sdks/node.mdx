...
import AgentWithSchemaNode from '/snippets/ja/v2/agent/with-schema/js.mdx'
import AgentStatusNode from '/snippets/ja/v2/agent/status/js.mdx'
...
各実行では、実行トレースと出力スナップショットも記録されます。これらは `getAgentTrace` と `getAgentSnapshot` で取得できます。イベントスキーマとすべてのパラメータについては、[Agent](/ja/features/agent) を参照してください。
