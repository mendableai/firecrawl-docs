...
* `checkPromptInjection`: 任意のブール値 (デフォルトは `false`) 。有効にすると、抽出を実行する前にスクレイピングしたページコンテンツでプロンプトインジェクションの試みをスキャンします。インジェクションが検出された場合、リクエストは403とエラーコード `SCRAPE_PROMPT_INJECTION_DETECTED` で失敗します。チェックの実行時に4クレジットが追加されます。[プロンプトインジェクションの検出](/ja/features/llm-extract#prompt-injection-detection)を参照してください。