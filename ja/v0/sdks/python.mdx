...
<Warning>
  **このページは、2025 年 4 月 1 日に廃止された v0 API に関するドキュメントです。** 過去の参照用として
  残されており、現在の Firecrawl の動作を説明するものではありません。最新の
  ドキュメントは [Python SDK](/ja/sdks/python) をご覧ください。既存の v0 連携を移行するには、
