...
<Warning>
  **このページは、2025年4月1日に廃止された v0 API について説明しています。** 過去の参考資料として残しているもので、現在の Firecrawl の動作を説明するものではありません。最新のドキュメント: [Go SDK](/ja/sdks/go)。既存の v0 連携を移行する場合は、[v0 からの移行](/ja/migrating-from-v0) を参照してください。
</Warning>
