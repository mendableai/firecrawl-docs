...
<Warning>
  **このページは 2025 年 4 月 1 日に廃止された v0 API のドキュメントです。** 過去の参照用として残されているもので、現在の Firecrawl の動作を説明するものではありません。最新のドキュメントは
  [高度なスクレイピングガイド](/ja/advanced-scraping-guide) をご覧ください。既存の v0 インテグレーションを移行する場合は、
  [v0 からの移行](/ja/migrating-from-v0) を参照してください。
</Warning>
