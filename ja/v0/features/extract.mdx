...
<Warning>
  **このページは2025年4月1日に廃止されたv0 APIについて説明しています。** 過去の参照用として残しているもので、現在のFirecrawlの動作を説明するものではありません。最新のドキュメントは[抽出](/ja/features/extract)をご覧ください。既存のv0連携を移行する場合は、[v0からの移行](/ja/migrating-from-v0)をご覧ください。
</Warning>
