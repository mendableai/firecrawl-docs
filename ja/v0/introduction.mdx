...
<Warning>
  **このページは 2025 年 4 月 1 日に非推奨となった v0 API のドキュメントです。** 過去の参照用として残しているもので、現在の Firecrawl の動作を説明するものではありません。最新のドキュメントは [クイックスタート](/ja/quickstart) をご覧ください。既存の v0 連携を移行する場合は、[v0 からの移行](/ja/migrating-from-v0) を参照してください。
</Warning>
