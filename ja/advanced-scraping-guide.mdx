...
| `pageMarkers` | `boolean`                   | `false`      | ドキュメントの Markdown 内の改ページ位置に `<!-- page N -->` マーカーを付加します (ページ間のみ。改ページをまたいで結合されたページでは番号が飛ぶ場合があります — [解析](/ja/features/parse#page-markers-pdf) を参照)。追加コストなし。 |
...
[Agent](/ja/features/agent) は、このエンドポイントの正規の参照先であり、実行トレース、webhook、完全なパラメータリストが含まれます。
...
パターンは Rust の正規表現 (RE2 形式) 構文を使用するため、先読み・後読みや後方参照はサポートされません。コンパイルできないパターンは `400` で拒否されます。各フィールドで指定できるのは最大 1000 パターン、1 パターンあたり最大 2000 文字までで、`includePaths` と `excludePaths` を合わせて最大 1000 パターン・100,000 文字までです。キーワードで絞り込む場合は、複数の語を 1 つの長い選択構文にまとめるのではなく、語ごとに短いパターンを 1 つずつ送信してください。
...
<div id="allowing-firecrawl-to-scrape-your-website">
  ### Firecrawl に Web サイトのスクレイピングを許可する
</div>

* **robots.txt**: Firecrawl はサイトをクロールする前に `robots.txt` を読み取り、user-agent トークン `FirecrawlAgent` 向けのルールと `*` 向けのルールを適用します。`robots.txt` で Firecrawl を許可または拒否する場合は `FirecrawlAgent` を指定してください。
* **User agent**: `FirecrawlAgent` は、Firecrawl が `robots.txt` のルールを適用する際に照合するトークンです。フェッチ時の `User-Agent` ヘッダーも同じ値であるとは考えないでください。Firecrawl を許可リストに追加する場合は、`User-Agent` 文字列で照合するファイアウォールや WAF のルールではなく、`robots.txt` のディレクティブを使用してください。
* **IP アドレス**: Firecrawl は送信元 IP アドレスを固定していません。
