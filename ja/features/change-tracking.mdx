...
変更追跡は、スクレイピングごとの差分を扱うプリミティブです。Firecrawl にスケジュールでチェックを実行させ、変更があったときに webhook、メール、Slack で通知してほしい場合は、自分でタイマーを組んで `/scrape` を呼び出すのではなく、[モニタリング](/ja/features/monitoring) を使ってください。
...
変更追跡は、定期的にスクレイピングを行う場合に最も効果を発揮します。[Monitoring](/ja/features/monitoring) を使えば、最短5分間隔でスケジュール実行を代行し、webhook、メール、Slack への通知を送信します。cron、クラウドスケジューラ、ワークフローツールなどで自分でスケジュールを管理したい場合は、以下のオプションをご利用ください。
