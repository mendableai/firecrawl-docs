...
[Page](/ja/features/monitoring-page) と [Web サイト](/ja/features/monitoring-website) のモニターでは、デフォルトで各ページの Markdown の差分を比較し、`same`、`changed`、`new`、`removed`、`error` のいずれかを返します。自分でスクレイピングを実行する場合は、[変更追跡](/ja/features/change-tracking) フォーマットを使って単一の呼び出しで同じ差分比較を利用できます。**特定の構造化フィールド** (価格、見出し、在庫フラグ、リスト内の項目など) の変更を検出したい場合は、ターゲットの `scrapeOptions` に `modes: ["json"]` を指定した `changeTracking` フォーマットを追加して、JSONモードの変更追跡を有効にします。
...
エンドポイントは 10 秒以内に `2xx` ステータスを返す必要があります。配信に失敗した場合は 1 分後、5 分後、15 分後の 3 回リトライされ、その後 webhook は失敗としてマークされます。完全な一覧は [Timeouts and retries](/ja/webhooks/overview#timeouts--retries) を参照してください。
...
チェック一覧は、チェックの `status` で絞り込めます: `queued`、`running`、`completed`、`failed`、`partial`、`skipped_overlap`、`skipped_no_credits`。これで全種類です。2つのスキップ系ステータスの意味については、[スキップされたチェックと一時停止中のモニター](#skipped-checks-and-paused-monitors)を参照してください。
...
<div id="skipped-checks-and-paused-monitors">
  ## スキップされたチェックと一時停止中のモニター
</div>

スケジュールされたチェックが実行されないまま終了することがあります。この場合は失敗ではなくスキップのステータスとして記録され、課金も発生しません。

| ステータス               | 内容                                                                                                                                           |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `skipped_overlap`    | このモニターの前回のチェックがまだ実行中のまま次のチェックの実行時刻を迎えたため、並行実行せずにスキップされました。次回のスケジュールされたチェックは通常どおり実行されます。                                                   |
| `skipped_no_credits` | このモニターのクレジットチェックが実行を拒否しました。`actualCredits` は `0` となり、`error` に理由が示されます。これは、teamのクレジットが不足している場合と、パートナーから付与されたジョブをパートナーが取り下げた場合の両方が該当します。 |

`skipped_no_credits` のチェックは、原因が解消され次第、自動的に解消されます。クレジットを追加するだけで十分で、モニター側でresetする必要はありません。

<div id="there-is-no-failure-counter-to-read">
  ### 読み取れる失敗カウンターはありません
</div>

Firecrawl は、モニターの失敗回数の累計、エラー率、連続スキップ数を保持も返却もしません。モニター オブジェクトが返すのは `status`、`nextRunAt`、`lastRunAt`、`currentCheckId`、`lastCheckSummary` で、チェック は自身の `status` と `error` を返します。いずれも累計値ではありません。

連続回数や発生率が必要な場合は、チェック の一覧から算出してください。`GET /v2/monitor/{monitorId}/checks` は チェック を新しい順に返すため、先頭から続く同一 ステータス の件数を数えれば現在の連続回数が分かり、異なる ステータス のチェック が 1 件現れた時点でその連続は途切れます。

<div id="automatic-pausing">
  ### 自動的な一時停止
</div>

Firecrawl がモニターを一時停止するのは、次の 1 つのケースだけです。[パートナー連携](/ja/partner-credits)を通じて付与されたクレジットでモニターが動作しているときに、パートナーがそのジョブを取り下げた場合、すべてのチェックは `skipped_no_credits` でスキップされ、パートナーがジョブを取り消した旨のエラーが記録されます。この状態が自然に解消されることはないため、この種のスキップが 3 回連続すると、Firecrawl はモニターの `status` を `paused` に設定し、`nextRunAt` をクリアしてスケジュールを停止します。

ここから次の 2 点が言えます。

* クレジットを使い切ってもモニターは一時停止されません。残高を超過したチームでもスケジュールは維持され、クレジットが再び利用可能になるまで `skipped_no_credits` のチェックが記録され続けます。
* 一時停止は状態の変化であり、通知ではありません。一時停止用の webhook イベントもメール通知もありません。確認するにはモニターの `status` を参照し、元のアクセス権が復旧したら `PATCH /v2/monitor/{monitorId}` で `active` に戻してください。

一時停止されたモニターは、構成とチェック履歴をそのまま保持します。一時停止は削除ではありません。
