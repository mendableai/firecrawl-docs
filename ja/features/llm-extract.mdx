...
* `checkPromptInjection`: 任意のブール値 (デフォルトは `false`) 。有効にすると、Firecrawl は抽出を実行する前に、スクレイピングしたページコンテンツにプロンプトインジェクションの試みがないかスキャンします。[プロンプトインジェクションの検出](#prompt-injection-detection)を参照してください。
...
Web ページには、LLM ベースの抽出を乗っ取るよう作成された隠しテキストが含まれている場合があります。たとえば、スキーマを無視して攻撃者が制御するデータを返すようモデルに指示するテキストです。信頼できない URL やユーザー送信の URL から抽出する場合は、抽出の実行前にスクレイピングしたコンテンツをチェックするオプトインの保護機能を有効にできます:
...
* 専用の分類器呼び出しがスクレイピングされたページコンテンツを検査します (抽出と並行して実行されるため、有効にしても問題のないスクレイピングが遅くなることはありません) 。
* プロンプトインジェクションの試みが検出されると、リクエストは HTTP `403` とエラーコード `SCRAPE_PROMPT_INJECTION_DETECTED` で失敗し、抽出結果は返されません。
* チェックが実行される場合、標準の JSON フォーマットのコストに加えて **+4 クレジット**が課金されます。チェックの実行後にスクレイピングが失敗した場合 (インジェクションが検出されてリクエストがブロックされた場合を含む) 、分類器呼び出しはすでに実行されているため、失敗したスクレイピングに通常課金される 0 クレジットではなく、**5 クレジット**が課金されます。

v1 では、同じオプションを `jsonOptions.checkPromptInjection` として利用できます。また、すべての公式 SDK でも利用できます (例: JS SDK の `checkPromptInjection`、Python SDK の v2 JSON フォーマットの `check_prompt_injection`) 。
