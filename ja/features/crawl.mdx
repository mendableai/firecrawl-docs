...
到達範囲は設定した スコープ パラメータによって決まります。詳細は [設定リファレンス](#configuration-reference) と [クロールエンドポイントリファレンス](/ja/api-reference/endpoint/crawl-post) を参照してください:
...
* **パスのパターンは Rust の正規表現 (RE2 形式) で、事前に検証されます。** 先読み・後読みや後方参照はサポートされておらず、コンパイルできないパターンは黙って無視されるのではなく `400` で拒否されます。各フィールドには最大 1000 個、1 個あたり最大 2000 文字までのパターンを指定でき、`includePaths` と `excludePaths` を合わせて最大 1000 個のパターンかつ 100,000 文字までです。キーワード的な絞り込みを行う場合は、複数の語を 1 つの長い選択パターンにまとめるのではなく、語ごとに短いパターンを 1 つずつ送信してください (`["pricing", "docs", "blog"]`) 。
* **サイトマップ モード。** デフォルトの `sitemap: "include"` では、URL は サイトマップ と再帰的なリンク探索の両方から取得されます。`"skip"` は HTML のリンクのみを使用するため、PDF や深い階層のページなど サイトマップ にしかないページは取得できません。`"only"` は サイトマップ と開始 URL をクロールし、HTML からのリンク探索は行いません。
...
`maxConcurrency` のデフォルトは、プランによって決まるチームの 同時実行数の上限 です — [Rate limits](/ja/rate-limits) を参照してください。
...
| `maxDiscoveryDepth`     | `integer`  | (none)      | URL 内の `/` セグメント数ではなく、リンクの検出ホップ数に基づくルート URL からの最大深度。ページ上で新しい URL が見つかるたびに、その URL には発見元のページより 1 つ深い深度が割り当てられます。ルートサイトおよびサイトマップに含まれるページの発見深度は 0 です。最大深度のページもスクレイプされますが、そのページ上のリンクはたどりません。 |
| `includePaths`          | `string[]` | (none)      | 含める URL パス名の正規表現パターン。一致するパスのみをクロールします。パターンは最大 1000 個、各 2000 文字までです。合計の上限と正規表現の構文については [スコープ](#what-the-crawler-is-scoped-to-reach) を参照してください。                                            |
| `excludePaths`          | `string[]` | (none)      | クロール対象から除外する URL パス名の正規表現パターン。上限は `includePaths` と同じです。                                                                                                                                  |
...
| `maxConcurrency`        | `integer`  | (none)      | 同時スクレイピングの最大数。デフォルトでは、チームの同時実行数の上限が使用されます。                                                                                                                                                 |
