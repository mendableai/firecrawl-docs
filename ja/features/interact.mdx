...
Interact は次のような場合に使用します:

* **ログインが必要なページをスクレイピング** — 一度サインインすれば、authentication 後にのみ表示されるページを読み取れます。[Persistent Profiles with Scrape + Interact](#persistent-profiles-with-scrape-+-interact) を参照してください。
* **Pagination をクリックして進む** — ページを送りながら各結果セットを取得し、1つの セッション を使い回します。[Interact via prompting](#interact-via-prompting) と [Session Lifecycle](#session-lifecycle) を参照してください。
* **フォームの入力と送信** — prompt から、または Playwright のコードから、フィールドに入力して送信します。[Interact via prompting](#interact-via-prompting) と [Running Code](#running-code) を参照してください。
* **authentication 済みの セッション を再利用** — browser の状態を名前付きプロファイルに保存し、以降のスクレイピングで読み込みます。[Persistent Profiles with Scrape + Interact](#persistent-profiles-with-scrape-+-interact) を参照してください。
* **スクレイピングを介さずに セッション を開始** — 直接操作できる独立したブラウザを開きます。[Browser Sandbox](/ja/features/browser) を参照してください。
...
| ------------------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| 最初にスクレイピングせず、独立したブラウザセッションを開始する | Browser Sandbox / 独立した Interact セッション | [Browser Sandbox](/ja/features/browser), [Create Browser Session](/ja/api-reference/endpoint/browser-create), [Execute Browser Code](/ja/api-reference/endpoint/browser-execute), [List Browser Sessions](/ja/api-reference/endpoint/browser-list), [Delete Browser Session](/ja/api-reference/endpoint/browser-delete) | `browser()`, `browserExecute()`, `listBrowsers()`, `deleteBrowser()` |
| `scrapeId` を使ってスクレイピング結果から続行する  | スクレイピング後に Interact                    | [Execute Interact](/ja/api-reference/endpoint/scrape-execute), [Stop Interact](/ja/api-reference/endpoint/scrape-browser-delete)                                                                                                                                                                               | `interact()`, `stopInteraction()`                                    |
...
<Warning>
  不要な課金を避けるため、使用後は必ずセッションを停止してください。Credits は秒単位で按分されます。最低料金はブラウザ1分です。`prompt` を使用するセッションはブラウザ1分あたり7 Credits、prompt を使用しないセッションは2 Credits が課金されます。詳細は[課金](/ja/billing#credit-costs-per-endpoint)を参照してください。
</Warning>
