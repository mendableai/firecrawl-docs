...
* **ログインが必要なページをスクレイピング** — 一度サインインすれば、authentication 後にのみ表示されるページを読み取れます。[Scrape + Interact での永続プロファイル](#persistent-profiles-with-scrape-+-interact) を参照してください。
* **Pagination をクリックして進む** — ページを送りながら各結果セットを取得し、1つの セッション を使い回します。[プロンプトで操作する](#interact-via-prompting) と [セッションのライフサイクル](#session-lifecycle) を参照してください。
* **フォームの入力と送信** — prompt から、または Playwright のコードから、フィールドに入力して送信します。[プロンプトで操作する](#interact-via-prompting) と [コードの実行](#running-code) を参照してください。
* **authentication 済みの セッション を再利用** — ブラウザの状態を名前付きプロファイルに保存し、以降のスクレイピングで読み込みます。[Scrape + Interact での永続プロファイル](#persistent-profiles-with-scrape-+-interact) を参照してください。
