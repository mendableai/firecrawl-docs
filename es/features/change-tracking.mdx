...
El seguimiento de cambios es la primitiva de diff por cada scrape. Si quieres que Firecrawl ejecute las comprobaciones de forma programada y te avise por webhook, correo electrónico o Slack cuando algo cambie, usa la [supervisión](/es/features/monitoring) en lugar de llamar a `/scrape` por tu cuenta con un temporizador.
...
El seguimiento de cambios es más útil cuando haces scraping a intervalos regulares. [Monitoring](/es/features/monitoring) ejecuta esa programación por ti, con intervalos de hasta 5 minutos, y envía notificaciones por webhook, correo electrónico o Slack. Usa las opciones a continuación cuando quieras gestionar tú mismo la programación con cron, planificadores en la nube o herramientas de orquestación de flujos de trabajo.
...
Para operaciones asíncronas como `crawl` y *batch scrape*, usa [webhooks](/es/webhooks/overview) para recibir resultados de seguimiento de cambios a medida que llegan en lugar de hacer *polling*.
...
El conjunto completo de opciones disponibles al pasar un objeto de formato `changeTracking`:
