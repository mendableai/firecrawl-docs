...
* `research`: Limita la búsqueda web a **sitios web** académicos y de investigación (arxiv.org, nature.com, ieee.org, pubmed.ncbi.nlm.nih.gov, biorxiv.org, medrxiv.org y similares). Devuelve resultados de páginas web normales con fragmentos, no registros de artículos. Para buscar los propios artículos, usa el [Research Index](/es/features/research)
* `pdf`: Busca archivos PDF
* `developer`: Busca en el [Developer Index](/es/features/developer): issues, pull requests fusionadas y archivos README de repositorios de código públicos, además de sitios de documentación seleccionados

<Note>
  **`research` es un filtro de sitios web, no el índice de artículos.** Limita la búsqueda web normal a una lista fija de dominios académicos y devuelve fragmentos de páginas de esos dominios.

  Si quieres buscar literatura científica —resúmenes completos, lectura de pasajes dentro de artículos y expansión del grafo de citas en un índice de artículos de PubMed, bioRxiv, medRxiv y arXiv—, usa el [Research Index](/es/features/research).
</Note>
