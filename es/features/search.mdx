...
* `research`: restringe la búsqueda web a sitios web académicos y de investigación (arxiv.org, nature.com, pubmed.ncbi.nlm.nih.gov y similares). A partir del 2026-11-16 pasará a buscar en el [Research Index](/es/features/research) y a devolver registros de artículos; consulta la advertencia más abajo
* `pdf`: busca PDF
* `developer`: busca en el [Developer Index](/es/features/developer): issues, pull requests fusionados y READMEs de repositorios de código públicos, junto con sitios de documentación seleccionados
...
<Warning>
  **La categoría `research` cambia el 2026-11-16.** Buscará en el [Research Index](/es/features/research) (PubMed, bioRxiv, medRxiv, arXiv) en lugar de limitar los resultados web a 14 sitios académicos. Los resultados pasarán de `data.web` a `data.research` y se devolverán como registros de artículos: `paperId`, `primaryId`, `ids`, `title`, `abstract`, `score`. Hasta entonces, cada respuesta que la utilice incluirá una entrada `warnings`.
  ...
  Si necesitas registros de artículos, actualiza tu parsing antes de esa fecha o llama hoy mismo a [`GET /search/research/papers`](/es/api-reference/endpoint/research-search-papers). Si lo que necesitas son páginas web de sitios académicos, cambia a [`includeDomains`](#domain-filters).
</Warning>
...
Restringe la búsqueda web a sitios web académicos y de investigación. Devuelve páginas web alojadas en esos dominios —páginas de inicio, páginas de resúmenes y páginas de editoriales— con los fragmentos habituales:
...
Para buscar en los propios artículos en lugar de en los sitios web que los alojan, usa el [Research Index](/es/features/research), que busca en los resúmenes de artículos de PubMed, bioRxiv, medRxiv y arXiv y puede leer pasajes dentro de un artículo:
...
Busca en el [Developer Index](/es/features/developer) fuentes primarias para responder a una pregunta de programación:
...
Los resultados para desarrolladores se devuelven en el grupo `web` estándar, cada uno con `category: "developer"`; la categoría `developer` no se puede combinar con otras categorías. Para obtener resultados ordenados con los pasajes coincidentes y usar los filtros de repositorio y fuente de documentación, utiliza el [endpoint de búsqueda para desarrolladores](/es/features/developer#search-the-developer-index).
  ...
  "data": {
    "web": [
      ...
      },
      {
        ...
        "title": "Revisión de redes neuronales",
        "description": "Una revisión de las arquitecturas de redes neuronales",
        "category": "pdf"
      }
    ]
  }
...
<div id="combining-search-zdr-with-scrape-zdr">
  ### Combinar búsqueda ZDR con Scrape ZDR
</div>

Si estás usando search con scraping de contenido (`scrapeOptions`), el parámetro `enterprise` aplica automáticamente ZDR a cualquier scraping resultante. La siguiente solicitud de ejemplo aplica ZDR tanto a la parte de búsqueda como a la de scraping del proceso:
...
Para más detalles sobre las opciones de scraping, consulta la [documentación de la función Scrape](https://docs.firecrawl.dev/features/scrape). Todo, excepto FIRE-1 (Agente) y el seguimiento de cambios, es compatible con este punto de conexión de búsqueda.
...
<div id="measured-performance">
  ## Rendimiento medido
</div>

Evaluadores independientes han comparado Firecrawl búsqueda con otras APIs de búsqueda. Las cifras siguientes se toman directamente de los resultados publicados por cada evaluador en la fecha indicada.

| Métrica                                     | Valor                                                             | Fuente                                                                         | Fecha                                               | n / ±                                                                                                                                                                                                                                      |
| ------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------ | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Agent Score (media de relevancia × calidad) | 14,58, puesto 2 de 8 (media de relevancia 4,30/5, calidad 3,39/5) | [AIMultiple — Agentic búsqueda Benchmark](https://aimultiple.com/agentic-search) | Dic 2025 (fuente actualizada el 25 de mayo de 2026) | IC del 95 % 13,12–15,98, n=100 consultas del ámbito de IA/LLM extraídas del propio tráfico de búsqueda orgánica de AIMultiple (10.000 remuestreos bootstrap); la fuente advierte que los resultados no son generalizables a otros dominios |

<Note>
  Se trata de mediciones de terceros, cada una con su propio harness, conjunto de tareas y metodología: no son directamente comparables entre sí ni con los benchmarks propios de Firecrawl. La estimación puntual la sitúa en el puesto 2 de 8; las pruebas de bootstrap emparejado no hallaron diferencias estadísticamente significativas frente a Brave, Exa o Parallel búsqueda Pro. Las páginas de benchmarks propios de Firecrawl, incluida la evaluación de código abierto del Developer Index, están en [firecrawl.dev/benchmarks](https://www.firecrawl.dev/benchmarks).
</Note>
