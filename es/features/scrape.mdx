...
¿Necesitas hacer scraping de un documento en una URL o procesar uno desde el disco? Aquí se explica cuándo usar scraping y cuándo usar [Parse](/es/features/parse):
...
<Info>
  Cada scrape consume 1 crédito. Se aplican créditos adicionales para ciertas opciones: el modo JSON cuesta 4 créditos adicionales por página, los formatos de question y highlights cuestan 4 créditos adicionales por página por formato, la redacción de PII cuesta 4 créditos adicionales por página, el procesamiento de PDF cuesta 1 crédito por página de PDF y la extracción de audio o video cuesta 4 créditos adicionales por página.
</Info>
...
<div id="response-metadata-and-status-codes">
  ### Metadatos de la respuesta y códigos de estado
</div>

Hay dos niveles de estado independientes en una respuesta de scraping, y conviene distinguirlos:

* **Estado de la solicitud a la API**: el estado HTTP de la propia llamada a la API de Firecrawl, junto con el valor booleano `success` en el cuerpo de la respuesta. Una solicitud aceptada y procesada devuelve HTTP `200` con `success: true`, aunque la página objetivo haya devuelto un código distinto de 2xx.
* **Estado de la página** (`data.metadata.statusCode`): el estado HTTP que el sitio web objetivo devolvió para esa página (p. ej., `200`, `301`, `404`, `403`, `500`). Indica cómo respondió la propia página.

Por tanto, el patrón fiable es: primero, confirme que la solicitud se completó correctamente (HTTP `200` + `success: true`) y, después, revise `data.metadata.statusCode` para conocer el resultado real de la página. Firecrawl considera que un `statusCode` de 2xx o `304` indica que la página se cargó correctamente; cualquier otro valor significa que no fue así. Cuando una página devuelve un estado de error, `data.metadata.error` puede incluir información adicional.
...
* Base64 sin procesar (`rawBase64`) - cuerpo de respuesta HTTP original codificado en Base64; debe ser el único formato de la solicitud
...
* Audio (`audio`) - extrae audio MP3 de URL de video compatibles, p. ej., YouTube (devuelve una URL firmada de GCS, caduca después de 1 hora)
* Video (`video`) - extrae video de la mejor calidad de URL de video compatibles, p. ej., YouTube (devuelve una URL firmada de GCS, caduca después de 1 hora)
...
El formato `rawBase64` devuelve el cuerpo de respuesta HTTP original codificado en Base64. El campo `rawBase64` contiene una cadena Base64 sin prefijo, no una URI de datos. Lee el tipo MIME en `metadata.contentType`. Solicita solo `rawBase64`, ya que la API lo rechaza si se combina con cualquier otro formato. La respuesta no incluye los campos `markdown`, `html` ni `rawHtml`, y contiene únicamente el archivo de esa URL, no CSS, imágenes ni otros subrecursos.
