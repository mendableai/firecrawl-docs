...
Usando el formato `changeTracking`, puedes supervisar cambios en un sitio web y recibir información sobre:
...
Para usar el seguimiento de cambios en el SDK de JavaScript, incluye `'changeTracking'` en el array de formatos al hacer scraping de una URL:
...
Para usar el seguimiento de cambios en el SDK de Python, incluye &#39;changeTracking&#39; en la lista de formatos al extraer una URL:
...
const result = await app.scrapeUrl('https://example.com/product', {
  formatos: ['markdown', 'changeTracking'],
  changeTrackingOptions: {
    modes: ['json'],
    schema: {
      type: 'object',
...
Usar la funcionalidad básica de seguimiento de cambios y el modo `git-diff` no tiene costo adicional. Sin embargo, si usas el modo `json` para comparar datos estructurados, el scraping de la página costará 5 créditos por página.