...
| `rawBase64` | Cuerpo de la respuesta HTTP original codificado en Base64, como una cadena Base64 sin formato. Debe ser el único formato de la solicitud. El tipo MIME está en `metadata.contentType`. |
...
| Parameter | Type    | predeterminado | Description                                                                                             |
| --------- | ------- | -------------- | ------------------------------------------------------------------------------------------------------- |
...
| Propiedad     | Type                        | predeterminado  | Description                                                                                                                                                                                                                                                  |
| ------------- | --------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| `pages`       | `boolean`                   | `false`         | También devuelve markdown físico por página en el campo `pages` del documento. Sin costo adicional.                                                                                                                                                          |
| `blocks`      | `boolean`                   | `false`         | También devuelve bloques de diseño tipados por página (cuadros delimitadores normalizados, tipos de bloques, orden de lectura, rangos de caracteres de markdown) en el campo `blocks` del documento. Sin costo adicional.                                    |
| `pageMarkers` | `boolean`                   | `false`         | Anota los saltos de página en el markdown del documento con marcadores `<!-- page N -->` (solo entre páginas; la numeración puede omitir páginas combinadas a través de un salto — consulta [Parse](/es/features/parse#page-markers-pdf)). Sin costo adicional. |
...
[Agent](/es/features/agent) es la referencia canónica para este endpoint, incluidas las trazas de ejecución, los webhooks y la lista completa de parámetros.
...
| Parámetro               | Tipo      | Predeterminado     | Descripción                                                                                                                                                                                                                   |
| ----------------------- | --------- | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `prompt`                | `string`  | *(obligatorio)*    | Instrucciones en lenguaje natural que describen qué datos extraer (máx. 10.000 caracteres).                                                                                                                                   |
...
| `model`                 | `string`  | `"spark-2"`        | Modelo de IA a utilizar. Los modelos Spark 1 están obsoletos y actualmente se redirigen a `"spark-2"`.                                                                                                                        |
| `effort`                | `string`  | *(sin establecer)* | Presupuesto de razonamiento: `"low"`, `"medium"` o `"high"`. Cada ejecución se realiza en `"spark-2"`, por lo que puede enviar `effort` con o sin `model`.                                                                    |
...
Los patrones usan la sintaxis de expresiones regulares de Rust (estilo RE2), por lo que no se admiten look-around ni retrorreferencias; un patrón que no compila se rechaza con un `400`. Cada campo acepta como máximo 1000 patrones de hasta 2000 caracteres cada uno, y `includePaths` y `excludePaths` en conjunto pueden contener como máximo 1000 patrones y 100.000 caracteres. Para el filtrado basado en palabras clave, envía un patrón corto por término en lugar de combinar los términos en una única alternancia larga.
...
<div id="allowing-firecrawl-to-scrape-your-website">
  ### Permitir que Firecrawl haga scraping de tu sitio web
</div>

* **robots.txt**: Firecrawl lee el `robots.txt` antes de rastrear un sitio y aplica tanto las reglas definidas para el token de user-agent `FirecrawlAgent` como las definidas para `*`. Usa `FirecrawlAgent` para permitir o bloquear Firecrawl en el `robots.txt`.
* **User agent**: `FirecrawlAgent` es el token que Firecrawl compara al aplicar las reglas de `robots.txt`. No des por hecho que también sea la cabecera `User-Agent` de la propia solicitud. Incluye a Firecrawl en la lista de permitidos mediante una directiva de `robots.txt`, no con una regla de firewall o WAF que coincida con una cadena `User-Agent`.
* **Direcciones IP**: Firecrawl no usa un conjunto fijo de IP de salida.
