...
* **`research`**：将网页搜索限定在学术与科研类网站 (arxiv.org、nature.com、pubmed.ncbi.nlm.nih.gov 等) 。自 2026-11-16 起将改为搜索[研究索引](/zh/features/research)并返回论文记录，请参见下方警告
* **`pdf`**：搜索 PDF
* **`developer`**：搜索[开发者索引](/zh/features/developer)——公开代码仓库中的 issue、已合并的 pull request 和 README，以及经过精选的文档站点

<Warning>
  **`research` 分类将自 2026-11-16 起变更。** 它将搜索[研究索引](/zh/features/research) (PubMed、bioRxiv、medRxiv、arXiv) ，而不再把网页结果限定在 14 个学术网站。结果将从 `data.web` 移至 `data.research`，并以论文记录的形式返回：`paperId`、`primaryId`、`ids`、`title`、`abstract`、`score`。在此之前，凡使用该分类的响应都会附带一个 `warnings` 条目。

  如果你需要论文记录，请在该日期前更新解析逻辑，或现在就改用 [`GET /search/research/papers`](/zh/api-reference/endpoint/research-search-papers)。如果你需要来自学术网站的网页，请改用 [`includeDomains`](#domain-filters)。
</Warning>
