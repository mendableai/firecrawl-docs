...
| `pageMarkers` | `boolean`                   | `false`  | 使用 `<!-- page N -->` 标记在文档 Markdown 中标注分页 (仅位于页面之间；跨分页处合并的页面可能导致编号跳过 — 请参见 [解析](/zh/features/parse#page-markers-pdf)) 。无需额外成本。 |
...
[代理](/zh/features/agent) 是此端点的权威参考，包含执行追踪、Webhook 和完整参数列表。
...
模式采用 Rust 正则表达式 (RE2 风格) 语法，因此不支持环视和反向引用；无法编译的模式将返回 `400` 并被拒绝。每个字段最多接受 1000 个模式，每个模式最多 2000 个字符；`includePaths` 与 `excludePaths` 合计最多包含 1000 个模式、100,000 个字符。若要按关键词过滤，请为每个词分别发送一个简短模式，不要把多个词合并成一个冗长的选择分支。
...
* **robots.txt**：Firecrawl 在爬取站点前会读取 `robots.txt`，并同时应用针对 user-agent 令牌 `FirecrawlAgent` 编写的规则和针对 `*` 编写的规则。在 `robots.txt` 中使用 `FirecrawlAgent` 即可允许或禁止 Firecrawl。
* **User agent**：`FirecrawlAgent` 是 Firecrawl 在应用 `robots.txt` 规则时所匹配的令牌，但不要假定实际抓取请求的 `User-Agent` 头也是这个值。请通过 `robots.txt` 指令将 Firecrawl 加入允许列表，而不要依赖匹配 `User-Agent` 字符串的防火墙或 WAF 规则。
* **IP 地址**：Firecrawl 没有固定的出站 IP 范围。
