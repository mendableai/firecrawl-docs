...
| Option                    | Additional Cost      | 描述                                                                                                                                                                              |
| ------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PDF parsing               | +1 credit / PDF page | 从 PDF 文档中提取内容                                                                                                                                                                   |
| JSON 格式 (LLM 提取)          | +4 credits / page    | 使用 LLM 从页面中提取结构化 JSON 数据                                                                                                                                                        |
| Prompt 注入检查               | +4 credits / page    | 面向 JSON 格式的可选 `checkPromptInjection` 防护检查 (请参见 [Prompt 注入检测](/zh/features/llm-extract#prompt-injection-detection)) 。如果检查执行后抓取失败，将收取 5 额度，而非抓取失败通常收取的 0 额度。这包括因检测到注入而被 403 封禁的抓取请求。 |
| Zero Data Retention (ZDR) | +1 credit / page     | 确保除本次请求外不会持久保存任何数据 (参见 [Scrape ZDR](/zh/features/scrape#zero-data-retention-zdr))                                                                                                  |

这些费用加成可以叠加。例如，同时使用 JSON 格式 和 Zero Data Retention 抓取同一页面时，每页将消耗 **1 + 4 + 1 = 6 额度**。由于 Crawl 和 Search 端点在内部会对每个页面调用 scrape，这些端点同样适用上述费用加成。
