...
<Warning>
  **本页记录的是 v0 API，已于 2025 年 4 月 1 日弃用。** 保留此页仅供
  历史参考，并不代表 Firecrawl 当前的工作方式。最新
  文档：[Crawl](/zh/features/crawl)。如需迁移现有的 v0 集成，请参见
...
2. **递归遍历：**
   递归跟进每个链接，覆盖所有子页面。此处仅描述 v0 的爬取模型。关于当前 crawl 如何发现和覆盖 URL，请参见 [Crawl](/zh/features/crawl)。
