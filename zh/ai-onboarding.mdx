...
如果你在使用 AI 开发，Firecrawl 提供了多种资源来提升你的开发体验。Firecrawl 内置了 **技能**——自包含的知识包，AI 编码代理会自动发现并使用它们。只需一条安装命令，代理即可获得用于实时网页操作的 CLI 技能，以及用于产出可重复交付成果的工作流技能。将 Firecrawl 集成到应用代码中的构建技能则需要单独一步 (`firecrawl setup build`) 。Firecrawl 用户可以通过两种方式获取 API 密钥。请参见下方的[获取凭据](#get-credentials)。
...
Firecrawl **技能**是一组自包含的知识包，Codex、Claude Code、Cursor 和 OpenCode 等 AI 编码代理会自动发现并使用它们。只需一条安装命令，即可设置用于实时网页操作的 CLI 工具，以及用于产出可重复交付成果的工作流技能。用于将 Firecrawl 集成到应用代码中的构建技能则通过 `firecrawl setup build` 安装：

```bash
npx -y firecrawl-cli@latest init --all --browser
```

* `--all` 会跳过代理选择，并初始化检测到的每个代理
* `--browser` 会自动打开浏览器进行 Firecrawl 身份验证
* 还可使用 `--skip-auth`、`--skip-install`、`--skip-skills` 和 `--agent <name>`。请参见 `firecrawl init --help`。
...
```bash
firecrawl setup skills      # CLI / 核心技能
firecrawl setup build       # 构建技能
...
Firecrawl 技能分为三类。`init` 会安装 CLI 技能和工作流技能。运行 `firecrawl setup build` 可获取构建技能。每个部分都位于各自独立的 repo 中，因此可以独立演进：
...
* [`firecrawl/firecrawl-workflows`](https://github.com/firecrawl/firecrawl-workflows) — 用于产出可重复交付成果的工作流技能

**CLI 技能** — 用于代理会话期间进行实时网页操作：

| 技能                          | Purpose                          |
| --------------------------- | -------------------------------- |
| `firecrawl`                 | 整体 CLI 命令工作流                     |
...
| `firecrawl-monitor`         | 检测站点变更并接收通知                      |
| `firecrawl-parse`           | 将本地文档解析为 markdown                |
| `firecrawl-download`        | 将站点下载为本地文件                       |
| `firecrawl-research-index`  | 搜索论文、阅读段落并追踪引用                   |
| `firecrawl-developer-index` | 搜索 issue、pull request、README 和文档 |

上述安装包含两种索引技能。若只想将论文搜索技能添加到现有设置中——处理生物医学或科学文献时值得这样做——请运行：
...
CLI 技能和工作流技能通过上面的安装步骤获得。构建技能则需要单独一步：`firecrawl setup build`。区别在于后续的使用场景：
...
<Steps>
  ...
  <Step title="应用集成（构建技能）">
    当你在构建应用、代理或工作流，并需要通过代码调用 Firecrawl API 时，请选择这个。使用 `firecrawl setup build` 安装构建技能。它们可帮助你选择合适的端点、接入 SDK，并运行一次冒烟测试。
...
<AccordionGroup>
  <Accordion title="搜索 — 通过查询发现页面" icon="magnifying-glass">
    ...
    **适用场景：** 常规网页研究、查找文档、竞品分析、回答需要最新网页信息的问题。

    **不适用场景：** 科学文献。要查找论文、阅读论文中的段落或追踪引文，请改用[研究索引](/zh/features/research) — 使用 `firecrawl_research_*` MCP 工具、CLI 中的 `firecrawl research search-papers`，或 `firecrawl-research-index` 技能。搜索返回网页 snippet；研究索引返回包含完整摘要的论文记录。
  </Accordion>
