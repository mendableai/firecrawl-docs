...
import AgentWithSchemaNode from '/snippets/zh/v2/agent/with-schema/js.mdx'
import AgentStatusNode from '/snippets/zh/v2/agent/status/js.mdx'
...
每次运行还会记录执行追踪和输出快照，你可以使用 `getAgentTrace` 和 `getAgentSnapshot` 读取它们。事件 schema 和完整参数列表请参见 [Agent](/zh/features/agent)。
