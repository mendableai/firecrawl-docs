...
所有监控类型都遵循相同的工作流：选择一个或多个目标，设置计划，添加一个可选的自然语言目标描述，并在有重要变化时接收 Webhook、电子邮件或 Slack 通知。本页介绍通用配置。若要查看特定目标的设置和示例，请前往 [Page](/zh/features/monitoring-page)、[Website](/zh/features/monitoring-website) 或 [全网页规模](/zh/features/monitoring-web-scale) 监控页面。
...
[页面](/zh/features/monitoring-page) 和 [网站](/zh/features/monitoring-website) 监控默认会比较每个页面的 markdown 差异，并报告 `same`、`changed`、`new`、`removed` 或 `error`。如果你自己执行 抓取，也可以通过 [变更追踪](/zh/features/change-tracking) 格式在单次调用中获得同样的差异比较。如果你想检测**特定结构化字段** (如价格、标题、库存状态标记、列表中的条目等) 的变化，请在目标的 `scrapeOptions` 中添加 `changeTracking` 格式，并设置 `modes: ["json"]`，以启用 JSON 模式的变更追踪。
...
你的 endpoint 必须在 10 秒内返回 `2xx` 状态。投递失败会重试三次，分别在 1 分钟、5 分钟和 15 分钟后进行，之后该 webhook 会被标记为失败。完整表格请参见 [超时与重试](/zh/webhooks/overview#timeouts--retries)。
...
检查列表可按检查 `status` 过滤：`queued`、`running`、`completed`、`failed`、`partial`、`skipped_overlap` 或 `skipped_no_credits`。这就是全部取值。关于这两种跳过状态的含义，请参见[跳过的检查与已暂停的监控](#skipped-checks-and-paused-monitors)。
...
计划中的 检查 可能未运行就结束。这类情况会记录为跳过状态，而非失败，也不会计费。

| 状态                   | 发生了什么                                                                                      |
| -------------------- | ------------------------------------------------------------------------------------------ |
| `skipped_overlap`    | 本次 检查 到期时，该监控的上一次 检查 仍在运行，因此本次被跳过，而不是与其并行运行。下一次计划的 检查 会正常运行。                      |
| `skipped_no_credits` | 该监控的额度检查拒绝了此次运行。`actualCredits` 为 `0`，`error` 会说明原因。这既包括团队额度耗尽的情况，也包括由合作伙伴授予的任务被合作伙伴撤回的情况。 |

一旦原因消除，`skipped_no_credits` 的 检查 会自行恢复。补充额度即可，无需在监控上重置任何内容。

<div id="there-is-no-failure-counter-to-read">
  ### 没有可直接读取的失败计数器
</div>

Firecrawl 不会为监控保存或返回失败总次数、错误率或连续跳过次数。监控对象返回 `status`、`nextRunAt`、`lastRunAt`、`currentCheckId` 和 `lastCheckSummary`，单次检查返回自身的 `status` 和 `error`，其中没有任何一项是累计值。

如果需要连续次数或比率，可自行从检查列表中推算。`GET /v2/monitor/{monitorId}/checks` 按从新到旧返回检查记录，因此统计开头连续相同 `status` 的数量即为当前的连续次数，一旦出现状态不同的检查，连续即中断。

<div id="automatic-pausing">
  ### 自动暂停
</div>

Firecrawl 只在一种情况下暂停监控：当监控使用通过[合作伙伴集成](/zh/partner-credits)发放的额度运行，而合作伙伴撤回了该任务时，每次检查都会以 `skipped_no_credits` 被跳过，并返回一条提示合作伙伴已撤销该任务的错误。这种情况不会自行恢复，因此在连续三次出现此类跳过后，Firecrawl 会将该监控的 `status` 设为 `paused` 并清除 `nextRunAt`，从而停止调度。

由此可以得出两点。

* 额度耗尽不会导致监控暂停。余额不足的团队仍会保留其调度，并持续记录 `skipped_no_credits` 检查，直到额度再次可用。
* 暂停是一次状态变更，而非一种通知。没有暂停的 webhook 事件，也不会发送暂停邮件。请读取监控的 `status` 来查看状态，并在底层访问权限恢复后，使用 `PATCH /v2/monitor/{monitorId}` 将其设回 `active`。

被暂停的监控会保留其配置和检查历史。暂停不等于删除。
