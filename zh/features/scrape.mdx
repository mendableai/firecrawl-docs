...
详情请参阅 [抓取 Endpoint API 参考](https://docs.firecrawl.dev/api-reference/endpoint/scrape)。

<PlaygroundCTA />

要抓取某个 URL 上的文档，还是解析本地磁盘上的文档？下面介绍何时使用 抓取、何时使用 [Parse](/zh/features/parse)：
...
<Info>
  每次抓取会消耗 1 个额度。某些选项会产生额外消耗：JSON 模式每页额外消耗 4 个额度，question 和 highlights formats 每种格式每页额外消耗 4 个额度，PII 脱敏每页额外消耗 4 个额度，PDF 解析每个 PDF 页面额外消耗 1 个额度，音频或视频提取每页额外消耗 4 个额度。
</Info>
...
抓取响应中有两个相互独立的状态层级，需要注意区分：

* **API 请求状态**：Firecrawl API 调用本身的 HTTP 状态，以及响应正文中的 `success` 布尔值。只要请求被接受并处理，就会返回 HTTP `200` 和 `success: true`，即使目标页面实际返回的是非 2xx 状态码。
* **页面状态** (`data.metadata.statusCode`) ：目标网站为该页面返回的 HTTP 状态码 (例如 `200`、`301`、`404`、`403`、`500`) 。它反映的是页面本身的响应情况。

因此，可靠的做法是：先确认请求成功 (HTTP `200` + `success: true`) ，再检查 `data.metadata.statusCode`，了解页面的实际结果。Firecrawl 将 2xx 或 `304` 的 `statusCode` 视为页面加载正常；其他状态则表示页面未能正常加载。当页面返回错误状态时，`data.metadata.error` 可能会包含更多详细信息。

<div id="scrape-formats">
  ## 抓取格式
</div>

现在你可以选择输出所需的格式。你可以指定多个输出格式。支持的格式包括：

* Markdown (`markdown`)
* 摘要 (`summary`)
* HTML (`html`) - 页面 HTML 的清洗版本
* 原始 HTML (`rawHtml`) - 按页面返回的不经修改的 HTML
* 原始 Base64 (`rawBase64`) - 经过 Base64 编码的原始 HTTP 响应正文；必须是请求中的唯一格式
...
`rawBase64` 格式会返回经过 Base64 编码的原始 HTTP 响应正文。`rawBase64` 字段包含纯 Base64 字符串，而非数据 URI。请从 `metadata.contentType` 读取 MIME 类型。请仅请求 `rawBase64`，因为 API 会拒绝将其与任何其他格式一起使用。响应中不包含 `markdown`、`html` 或 `rawHtml` 字段，并且它仅包含该 URL 对应的单个文件，不包含 CSS、图片或其他子资源。
