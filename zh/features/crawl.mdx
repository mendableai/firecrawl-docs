...
* **`includePaths` / `excludePaths` 以正则表达式匹配 URL 的 URL 路径** —— 而非完整 URL，也不包含查询参数。设置 `regexOnFullURL: true` 可改为匹配包含查询字符串的完整 URL。起始 URL 同样会与 `includePaths` 进行匹配：若不匹配，爬取可能返回 0 个页面。
* **路径模式采用 Rust 正则 (RE2 风格) ，并会提前校验。** 不支持环视和反向引用，无法编译的模式会以 `400` 被拒绝，而不是被静默忽略。每个字段最多接受 1000 个模式，每个模式最多 2000 个字符；`includePaths` 与 `excludePaths` 合计最多包含 1000 个模式和 100,000 个字符。若要做关键词式过滤，请为每个词分别发送一个简短模式 (`["pricing", "docs", "blog"]`) ，而不要把多个词合并成一个很长的择一表达式。
...
* **遵守 robots.txt**，除非启用 `ignoreRobotsTxt` (仅限 Enterprise) 。

`maxConcurrency` 默认为你团队的并发限制，该上限由你的 plan 决定 —— 请参见[限流](/zh/rate-limits)。
...
| `includePaths`          | `string[]` | (无)         | 要包含的 URL 路径正则表达式模式。仅爬取匹配的路径。最多 1000 个模式，每个最长 2000 个字符；有关组合限制和正则语法，请参见 [scope](#what-the-crawler-is-scoped-to-reach)。                          |
| `excludePaths`          | `string[]` | (无)         | 要从爬取中排除的 URL 路径正则表达式模式。限制与 `includePaths` 相同。                                                                                                 |
...
| `delay`                 | `number`   | (无)         | 每次抓取之间的延迟时间 (秒) ，以遵守限流。设置此项会强制并发数为 1。                                                                                                       |
