...
如果你需要更彻底的递归发现，并希望获得可审计的结果与错误记录，而非快速的近似结果，请使用 [/crawl](/zh/features/crawl)：它通过递归链接遍历以及 sitemap 来发现页面，并报告返回了哪些内容、哪些抓取失败 —— 请参见[执行与结果统计](/zh/features/crawl#execution-and-result-accounting)。若只需要某个特定页面上的链接，请在 [/scrape](/zh/features/scrape) 中请求 [`links` 格式](/zh/features/scrape#scrape-formats)。
