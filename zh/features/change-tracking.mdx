...
变更追踪 会将页面的当前内容与上次抓取时的内容进行比较。将 `changeTracking` 添加到你的 `formats` 数组中，以检测页面是新增、未变化还是已修改，并 (可选) 获取结构化的差异信息，了解具体发生了哪些变更。

* 适用于 `/scrape`、`/crawl` 和 `/batch/scrape`
* 提供两种 diff 模式：用于行级变更的 `git-diff`，以及用于字段级比较的 `json`
* 作用范围限定在你的团队内，也可以按你传入的 tag 进一步限定

变更追踪 是单次抓取层面的 diff 原语。如果你希望 Firecrawl 按 schedule 定期执行检查，并在内容发生变更时通过 webhook、email 或 Slack 通知你，请使用[监控](/zh/features/monitoring)，而不是自己定时调用 `/scrape`。
...
当你按固定计划定期进行抓取时，变更追踪的效果最佳。[监控](/zh/features/monitoring)会替你执行该计划，间隔最短可至 5 分钟，并发送 webhook、邮件或 Slack 通知。如果你希望自行掌控调度计划，可使用下面的选项，通过 cron 任务、云调度服务或工作流工具来实现。
