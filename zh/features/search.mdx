...
* `research`：将网页搜索限定为学术与科研类网站 (arxiv.org、nature.com、ieee.org、pubmed.ncbi.nlm.nih.gov、biorxiv.org、medrxiv.org 等) 。返回带有 snippet 的普通网页结果，而非论文记录。要搜索论文本身，请使用 [研究索引](/zh/features/research)
* `pdf`：搜索 PDF
* `developer`：搜索 [Developer Index](/zh/features/developer)——涵盖公开代码仓库中的 issue、已合并的 pull request 和 README，以及精选文档网站

<Note>
  **`research` 是网站筛选器，而非论文索引。**它会将普通网页搜索限定在固定的学术域名列表中，并返回这些网站的页面 snippet。

  如果您想搜索科学文献——包括完整摘要、论文内段落，以及在涵盖 PubMed、bioRxiv、medRxiv 和 arXiv 的论文索引中扩展 citation 图谱——请改用 [研究索引](/zh/features/research)。
</Note>
